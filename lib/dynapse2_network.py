...
class Network:
    ...
    def __init__(self, config, profile_path, num_chips=1, tol=.1, boards = ["orange"]):
        ...
        print([profile_path + f"cams_profile_{board}.json" for board  in boards])
        self.prob_fail, self.cam_good, self.num_good_cams = \
            self.read_good_cam_list(filenames=[profile_path + f"cams_profile_{board}.json" for board  in boards], tol=tol)#,profile_path + "cams_profile_green.json" #"cams_profile_orange"
    ...
    def add_virtual_group(self, size,name=''):
        group = VirtualGroup(size=size,name=name)
    ...
    def add_connection(self, source, target,dendrite,probability=0, weight=None,
                       repeat=1, stp=False, precise_delay=False, mismatched_delay=False, matrix=None):
        if isinstance(source, list):
            assert set(source).issubset(self.groups) or set(source).issubset(self.virtual_groups), f"Source group {source} undefined in either neuron groups {self.groups} or virtual neuron groups {self.virtual_groups}"
        else:
            assert source in self.groups or source in self.virtual_groups, f"Source group {source} undefined in either neuron groups {self.groups} or virtual neuron groups {self.virtual_groups}"
    ...
    def config_connections(self):
        ...
        for group in self.groups:
            ...
            for i, neuron in enumerate(group.neurons):
                # if i < 5:
                #     conn = [(_.tag,_.weight) for _ in synapses[i] if _.tag > 0]
                #     print(i,neuron,len(conn),conn)
                #     print()
