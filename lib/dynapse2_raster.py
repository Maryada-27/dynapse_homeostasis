...
def GetFiringFrequency(ti,N,simTime,t_window=20.):#,startTime,endTime):,l,iteration,
    '''
    ti : spike train
    N : Neuron population
    simTime : to calculate bins   
    '''
    bins=np.arange(0,simTime,t_window)
    hists,_ =np.histogram(ti,bins=bins)
    fireRate= np.zeros(len(bins))
    idx_count=1

    for n in hists:
        fireRate[idx_count] =1000 * n / (t_window * N)
        idx_count+=1
    return fireRate,bins

def plot_raster(output_events,imgdir='',figname='',simD=None,ymax=None):
    # raster plot
    raster = [[] for _ in range(max(output_events[0]) + 1)]
    for i in range(len(output_events[0])):
        raster[output_events[0][i]] += [output_events[1][i]]
    plt.figure(figsize=(10, 5))
    for k in range(len(raster)):
        plt.plot(raster[k], [k] * len(raster[k]), '.', markersize=2)
    start = output_events[1][0]
    end =output_events[1][-1]
    if simD:
        end = start + simD
        plt.xlim(start-0.01,end + 0.01)
    if ymax:
        plt.ylim(-5,ymax+5)
    plt.grid(True)
    plt.ylabel('Neuron ID')
    os.makedirs(f'{imgdir}',exist_ok=True)
    plt.savefig(f'{imgdir}/{figname}.png')
    # plt.show()
    plt.close()